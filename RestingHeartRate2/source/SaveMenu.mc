...
class SaveMenu_MenuDelegate extends WatchUi.MenuInputDelegate{
	...
	function onMenuItem(item){
		if(item == :item_1){
			if(global_recordingSession.isRecording()){
				global_recordingSession.stop();
				global_recordingSession.save();
				global_recordingSession = null;
			}
			//saveHR();
			...
			if(global_recordingSession.isRecording()){
				global_recordingSession.stop();
				global_recordingSession.discard();
			}
			global_recordingSession = null;
			System.println("2");
			//Discard
