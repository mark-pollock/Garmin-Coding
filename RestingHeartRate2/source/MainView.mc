...
var global_recordingSession = null;
...
class MainView extends WatchUi.View {
    ...
    function onHide(){
    	...
    	bestAverageField.setData(global_bestAverage);
    	System.print("bestAverageField set to: ");
    	System.print(global_bestAverage);
    }
	...
	function initialize(){
        ...
        HR_graph = new LineGraph( 30, 10, Graphics.COLOR_RED );
        ...
        global_recordingSession = ActivityRecording.createSession(
			{
				:name=>"MyGenericSession",
				:sport=>ActivityRecording.SPORT_GENERIC
			}
		);
		var BEST_AVERAGE_ID = 0;
        bestAverageField = global_recordingSession.createField(
			"myAverage",
			BEST_AVERAGE_ID,
			FitContributor.DATA_TYPE_FLOAT,
			{:mesgType=>FitContributor.MESG_TYPE_SESSION, :units=>"beatsPerMinute"}
		);
		
		global_recordingSession.start();
	}
